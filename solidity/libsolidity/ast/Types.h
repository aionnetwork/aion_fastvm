...
public:
	...
	std::pair<u128, unsigned> const* offset(size_t _index) const;
	/// @returns the total number of slots occupied by all members.
	u128 const& storageSize() const { return m_storageSize; }

private:
	u128 m_storageSize;
	std::map<size_t, std::pair<u128, unsigned>> m_offsets;
};
...
public:
	...
	std::pair<u128, unsigned> const* memberStorageOffset(std::string const& _name) const;
	/// @returns the number of storage slots occupied by the members.
	u128 const& storageSize() const;
...
public:
	...
	/// returns the size of the pointer (usually 16). Returns 0 if the type cannot be encoded
	/// in calldata.
	/// If @a _padded then it is assumed that each element is padded to a multiple of 16 bytes.
	...
	virtual unsigned storageBytes() const { return 16; }
...
public:
	...
	virtual unsigned calldataEncodedSize(bool _padded = true) const override { return _padded ? (m_totalBits > 128 ? 32: 16) : m_totalBits / 8; }
...
public:
	...
	virtual unsigned calldataEncodedSize(bool _padded) const override{ return _padded ? 16 : 1; }
...
public:
	...
	virtual unsigned memoryHeadSize() const override { return 16; }
...
public:
	...
	virtual u256 storageSize() const override { return 2; }
	virtual bool canLiveOutsideStorage() const override { return true; }
	virtual unsigned sizeOnStack() const override { return m_super ? 0 : 2; }
	virtual bool isValueType() const override { return true; }
	virtual std::string toString(bool _short) const override;
	virtual std::string canonicalName(bool _addDataLocation) const override;
...
public:
	...
	virtual TypePointer encodingType() const override
	{
		return location() == DataLocation::Storage ? std::make_shared<IntegerType>(128) : TypePointer();
	}
	virtual TypePointer interfaceType(bool _inLibrary) const override;
	...
	std::pair<u128, unsigned> const& storageOffsetsOfMember(std::string const& _name) const;
	u128 memoryOffsetOfMember(std::string const& _name) const;
...
public:
	...
	virtual TypePointer binaryOperatorResult(Token::Value, TypePointer const&) const override { return TypePointer(); }
	virtual unsigned calldataEncodedSize(bool _padded) const override { (void)_padded; return 16; }
	virtual bool canBeStored() const override { return false; }
	virtual bool canLiveOutsideStorage() const override { return false; }
	virtual bool isValueType() const override { return true; }
