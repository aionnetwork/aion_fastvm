...
void ContractCompiler::appendFunctionSelector(ContractDefinition const& _contract)
{
	...
	for (auto const& it: interfaceFunctions)
	{
		...
		m_context << dupInstruction(1) << u128(FixedHash<4>::Arith(it.first)) << Instruction::EQ;
		...
		if (type->category() == Type::Category::Array)
		{
			...
			if (_fromMemory)
			{
				...
				else
				{
					...
					m_context << u128(arrayType.calldataEncodedSize(true)) << Instruction::ADD;
				}
			}
			else
			{
				...
				if (calldataType->isDynamicallySized())
				{
					...
					CompilerUtils(m_context).loadFromMemoryDynamic(IntegerType(128), !_fromMemory);
					...
					// retrieve length
					CompilerUtils(m_context).loadFromMemoryDynamic(IntegerType(128), !_fromMemory, true);
				...
				else
				{
					...
					m_context << Instruction::DUP1;
					m_context << u128(calldataType->calldataEncodedSize()) << Instruction::ADD;
				}
