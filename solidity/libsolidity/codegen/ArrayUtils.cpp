...
void ArrayUtils::copyArrayToStorage(ArrayType const& _targetType, ArrayType const& _sourceType) const
{
	...
	TypePointer uint128 = make_shared<IntegerType>(128);
	TypePointer targetBaseType = _targetType.isByteArray() ? uint128 : _targetType.baseType();
	TypePointer sourceBaseType = _sourceType.isByteArray() ? uint128 : _sourceType.baseType();
	...
	bool haveByteOffsetSource = !directCopy && sourceIsStorage && sourceBaseType->storageBytes() <= 8;
	bool haveByteOffsetTarget = !directCopy && targetBaseType->storageBytes() <= 8;
	...
	if (_sourceType.location() == DataLocation::Memory && _sourceType.isDynamicallySized())
	{
		...
		m_context << Instruction::SWAP1 << u128(0x10);  // array length is encoded as one word
	...
	m_context.callLowLevelFunction(
		...
		[=](CompilerContext& _context)
		{
			...
			{
				// stack: target_ref target_data_end source_length target_data_pos source_ref
				_context << Instruction::DUP3 << u128(15) << Instruction::LT;
				...
				else
				{
					...
					_context << u128(0xff) << Instruction::NOT << Instruction::AND;
				...
				_context << u128(1) << Instruction::ADD;
				_context << Instruction::DUP6 << Instruction::SSTORE;
			}
			...
			else
			{
				...
				solAssert(
					2 + byteOffsetSize + sourceBaseType->sizeOnStack() <= 32,
					"14-Stack too deep, try removing local variables."
				);
...
void ArrayUtils::copyArrayToMemory(ArrayType const& _sourceType, bool _padToWordBoundaries) const
{
	...
	if (_sourceType.location() == DataLocation::CallData)
	{
		...
		if (baseSize > 1)
			m_context << u128(baseSize) << Instruction::MUL;
	...
	else if (_sourceType.location() == DataLocation::Memory)
	{
		...
		{
			// copy using a loop
			m_context << u128(0) << Instruction::SWAP3;
			...
			m_context << Instruction::SWAP3 << u128(1) << Instruction::ADD;
		...
		if (_sourceType.isDynamicallySized())
		{
			...
			m_context << Instruction::SWAP1 << u128(16) << Instruction::ADD;
		...
		// We can resort to copying full 16 bytes only if
		// - the length is known to be a multiple of 16 or
		// - we will pad to full 16 bytes later anyway.
		if (((baseSize % 16) == 0) || _padToWordBoundaries)
			utils.memoryCopy16();
		else
			utils.memoryCopy();
		...
		if (_sourceType.isDynamicallySized())
			paddingNeeded = _padToWordBoundaries && ((baseSize % 16) != 0);
		else
			paddingNeeded = _padToWordBoundaries && (((_sourceType.length() * baseSize) % 16) != 0);
		if (paddingNeeded)
		{
			// stack: <target> <size>
			m_context << Instruction::SWAP1 << Instruction::DUP2 << Instruction::ADD;
			// stack: <length> <target + size>
			m_context << Instruction::SWAP1 << u128(15) << Instruction::AND;
			// stack: <target + size> <remainder = size % 16>
			...
			// Now we AND it with ~(2**(8 * (16 - remainder)) - 1)
			m_context << u128(1);
			m_context << Instruction::DUP4 << u128(16) << Instruction::SUB;
			// stack: ...<v> 1 <16 - remainder>
			m_context << u128(0x100) << Instruction::EXP << Instruction::SUB;
	...
	else
	{
		...
		if (_sourceType.isByteArray())
		{
			...
			// stack here: memory_offset storage_offset length
			// add 16 or length to memory offset
		...
		bool haveByteOffset = !_sourceType.isByteArray() && storageBytes <= 8;
		if (haveByteOffset)
		...
		else
		{
			...
			if (haveByteOffset)
				...
				m_context << Instruction::DUP2 << u128(0);
		...
		if (haveByteOffset)
			m_context << Instruction::SWAP1 << Instruction::POP;
		if (_padToWordBoundaries && baseSize % 16 != 0)
		{
			// memory_end_offset - start is the actual length (we want to compute the ceil of).
			// memory_offset - start is its next multiple of 16, but it might be off by 16.
			// so we compute: memory_end_offset += (memory_offset - memory_end_offest) & 15
			m_context << Instruction::DUP3 << Instruction::SWAP1 << Instruction::SUB;
			m_context << u128(15) << Instruction::AND;
			m_context << Instruction::DUP3 << Instruction::ADD;
			m_context << Instruction::SWAP2;
		}
...
void ArrayUtils::clearArray(ArrayType const& _typeIn) const
{
	...
	m_context.callLowLevelFunction(
		...
		[type](CompilerContext& _context)
		{
			...
			if (_type.baseType()->storageBytes() < 16)
			{
				solAssert(_type.baseType()->isValueType(), "Invalid storage size for non-value type.");
				solAssert(_type.baseType()->storageSize() <= 1, "Invalid storage size for type.");
			}
			if (_type.baseType()->isValueType())
				solAssert(_type.baseType()->storageSize() <= 2, "Invalid size for value type.");
			...
			else if (_type.baseType()->isValueType() && _type.storageSize() <= 5)
			{
				...
				for (unsigned i = 1; i < _type.storageSize(); ++i)
					_context
						<< u128(0) << Instruction::DUP2 << Instruction::SSTORE
						<< u128(1) << Instruction::ADD;
				_context << u128(0) << Instruction::SWAP1 << Instruction::SSTORE;
			}
			else if (!_type.baseType()->isValueType() && _type.length() <= 4)
			{
				...
				for (unsigned i = 1; i < _type.length(); ++i)
				{
					...
					_context
						<< Instruction::POP
						<< u128(_type.baseType()->storageSize()) << Instruction::ADD;
				}
				_context << u128(0);
			...
			else
			{
				...
				if (_type.baseType()->storageBytes() < 16)
					ArrayUtils(_context).clearStorageLoop(make_shared<IntegerType>(128));
				else
...
void ArrayUtils::clearDynamicArray(ArrayType const& _type) const
{
	...
	// set length to zero
	m_context << u128(0) << Instruction::DUP3 << Instruction::SSTORE;
	...
	if (_type.isByteArray() || _type.baseType()->storageBytes() < 16)
		clearStorageLoop(make_shared<IntegerType>(128));
	else
	...
	m_context.callLowLevelFunction(
		...
		[type](CompilerContext& _context)
		{
			...
			if (_type.isByteArray())
			{
				...
				_context << Instruction::DUP2 << u128(15) << Instruction::LT;
				...
				// Compute 1 << (128 - 8 * new_size)
				eth::AssemblyItem shortToShort = _context.newTag();
				_context << shortToShort;
				_context << Instruction::DUP3 << u128(8) << Instruction::MUL;
				_context << u128(0x80) << Instruction::SUB;
				_context << u128(2) << Instruction::EXP;
			...
			if (_type.isByteArray())
				...
				_context << Instruction::DUP1 << Instruction::ADD << u128(1) << Instruction::ADD;
			_context<< Instruction::DUP4 << Instruction::SSTORE;
...
void ArrayUtils::convertLengthToSize(ArrayType const& _arrayType, bool _pad) const
{
	if (_arrayType.location() == DataLocation::Storage)
	{
		if (_arrayType.baseType()->storageSize() <= 1)
		{
			...
			if (baseBytes == 0)
				m_context << Instruction::POP << u128(1);
			else if (baseBytes <= 8)
			{
				unsigned itemsPerSlot = 16 / baseBytes;
				m_context
					<< u128(itemsPerSlot - 1) << Instruction::ADD
					<< u128(itemsPerSlot) << Instruction::SWAP1 << Instruction::DIV;
			}
		}
		else
	...
	else
	{
		if (!_arrayType.isByteArray())
		{
			...
			m_context << u128(15) << Instruction::ADD
				<< u128(16) << Instruction::DUP1
	...
	else
	{
		...
		case DataLocation::Storage:
			...
			if (_arrayType.isByteArray())
			{
				...
				m_context << u128(1) << Instruction::DUP2 << u128(1) << Instruction::AND;
				m_context << Instruction::ISZERO << u128(0x100) << Instruction::MUL;
				m_context << Instruction::SUB << Instruction::AND;
				m_context << u128(2) << Instruction::SWAP1 << Instruction::DIV;
			}
			break;
		}
	}
...
void ArrayUtils::accessIndex(ArrayType const& _arrayType, bool _doBoundsCheck) const
{
	...
	case DataLocation::Storage:
	{
		...
		if (_arrayType.isByteArray())
		{
			...
			m_context << Instruction::SWAP1;
			m_context << Instruction::DUP2 << Instruction::SLOAD;
			m_context << u128(1) << Instruction::AND << Instruction::ISZERO;
		...
		if (_arrayType.baseType()->storageBytes() <= 16)
		{
			...
			unsigned itemsPerSlot = 16 / byteSize;
			m_context << u128(itemsPerSlot) << Instruction::SWAP2;
			...
			// stack: itemsPerSlot index (data_ref + index / itemsPerSlot)
				...
				<< Instruction::MOD;
			if (byteSize != 1)
				m_context << u128(byteSize) << Instruction::MUL;
		}
		else
		{
			if (_arrayType.baseType()->storageSize() != 1)
				m_context << _arrayType.baseType()->storageSize() << Instruction::MUL;
			m_context << Instruction::ADD << u128(0);
		}
		m_context << endTag;
		break;
	}
...
{
	solAssert(_byteSize < 16, "Invalid byte offset increment");
	solAssert(_byteSize != 0, "Invalid byte offset increment");
	// We do the following, but avoiding jumps:
	// byteOffset += byteSize
	// if (byteOffset + byteSize > 16)
	// {
	//     storageOffset++;
	//     byteOffset = 0;
	// }
	...
	m_context << u128(_byteSize) << Instruction::ADD;
	if (_byteOffsetPosition > 1)
		m_context << swapInstruction(_byteOffsetPosition - 1);
	// compute, X := (byteOffset + byteSize - 1) / 16, should be 1 iff byteOffset + bytesize > 16
	m_context
		<< u256(16) << dupInstruction(1 + _byteOffsetPosition) << u128(_byteSize - 1)
	...
	m_context << u128(1) << Instruction::SUB;
	// stack: 1 - X
	if (_byteOffsetPosition == 1)
		m_context << Instruction::MUL;
	else
