...
void ExpressionCompiler::appendStateVariableAccessor(VariableDeclaration const& _varDecl)
{
	...
	// stack: arg1 arg2 storage_offset byte_offset

	TypePointer returnType = _varDecl.annotation().type;

	unsigned paramTotalSize = 0;
	for (size_t i = 0; i < paramTypes.size(); ++i) {
		paramTotalSize += paramTypes[i]->sizeOnStack();
	}

	unsigned paramDepth = paramTotalSize;
	unsigned paramSize;
	for (size_t i = 0; i < paramTypes.size(); ++i, paramDepth -= paramSize)
	{
		paramSize = paramTypes[i]->sizeOnStack();
		...
		if (auto mappingType = dynamic_cast<MappingType const*>(returnType.get()))
		{
			...
			solAssert(paramSize == 1 || paramSize == 2, "Invalid stack size");

			// pop offset
			m_context << Instruction::POP;
			// move storage offset to memory.
			utils().storeInMemory(paramSize * 16);
			// move key to memory.
			utils().copyToStackTop(paramDepth, paramSize);
			utils().storeInMemory(0);
			if (paramSize == 2) {
				utils().storeInMemory(16);
			}
			m_context << u128(paramSize * 16 + 16) << u128(0) << Instruction::KECCAK256 << Instruction::SWAP1 << Instruction::POP;
			// push offset
			m_context << u128(0);
		...
		else if (auto arrayType = dynamic_cast<ArrayType const*>(returnType.get()))
		{
			// pop offset
			m_context << Instruction::POP;
			utils().copyToStackTop(paramDepth + 1 /* storage_offset */, paramSize);
	...
	else if (paramTotalSize >= 2)
	{
		m_context << swapInstruction(paramTotalSize);
		m_context << Instruction::POP;
		m_context << swapInstruction(paramTotalSize);
		utils().popStackSlots(paramTotalSize - 1);
	}
		...
		for (size_t i = 0; i < names.size(); ++i)
		{
			...
			if (auto arrayType = dynamic_cast<ArrayType const*>(returnTypes[i].get()))
				if (!arrayType->isByteArray())
					continue;
			pair<u128, unsigned> const& offsets = structType->storageOffsetsOfMember(names[i]);
			m_context << Instruction::DUP1 << u128(offsets.first) << Instruction::ADD << u128(offsets.second);
	...
	else
	{
		...
		solAssert(returnTypes.size() == 1 || returnTypes.size() == 2, "Return size can only be 1 or 2");
...
bool ExpressionCompiler::visit(TupleExpression const& _tuple)
{
	if (_tuple.isInlineArray())
	{
		...
		m_context << max(u256(16u), arrayType.memorySize());
...
bool ExpressionCompiler::visit(UnaryOperation const& _unaryOperation)
{
	...
	case Token::BitNot: // ~
		...
		if (_unaryOperation.annotation().type->sizeOnStack() == 2) {
			m_context << Instruction::SWAP1 << Instruction::NOT << Instruction::SWAP1;
		}
		break;
	...
	case Token::Add: // +
		...
		break;
	case Token::Sub: // -
		m_context << u128(0) << Instruction::SUB;
		break;
	default:
...
bool ExpressionCompiler::visit(FunctionCall const& _functionCall)
{
	...
	if (_functionCall.annotation().kind == FunctionCallKind::StructConstructorCall)
	{
		...
		m_context << max(u256(16u), structType.memorySize());
		utils().allocateMemory();
		m_context << Instruction::DUP1;
	...
	else
	{
		...
		case FunctionType::Kind::Creation:
		{
			...
			m_context << Instruction::DUP1 << Instruction::ISZERO
					<< Instruction::DUP3 << Instruction::ISZERO << Instruction::AND;
			m_context.appendConditionalRevert();

			if (function.valueSet())
				m_context << swapInstruction(2) << Instruction::POP;
			break;
		}
		case FunctionType::Kind::SetGas:
		{
			...
			utils().convertType(*arguments.front()->annotation().type, IntegerType(128), true);
		...
		case FunctionType::Kind::Transfer:
			...
			m_context << u128(eth::GasCosts::callStipend);
			arguments.front()->accept(*this);
			utils().convertType(
		...
		case FunctionType::Kind::Event:
		{
			...
			for (unsigned arg = arguments.size(); arg > 0; --arg)
				...
				{
					// numIndexed += function.parameterTypes()[arg - 1]->sizeOnStack();
					...
					else
					{
						...
						if ((*function.parameterTypes()[arg - 1]).sizeOnStack() == 1)
						{
							// append zeros based on the alignment of the parameter type
							bool leftAligned = (*function.parameterTypes()[arg - 1]).category() == Type::Category::FixedBytes;
							if (leftAligned) {
		...
		case FunctionType::Kind::MulMod:
		{
			for (unsigned i = 0; i < 3; i ++)
			{
				...
				utils().convertType(*arguments[2 - i]->annotation().type, IntegerType(128));
			}
		...
		case FunctionType::Kind::EDVerify:
		{
			...
			static const map<FunctionType::Kind, u128> contractAddresses{{FunctionType::Kind::ECRecover, 1},
															   ...
															   {FunctionType::Kind::RIPEMD160, 3},
																 {FunctionType::Kind::ECRecover, 200}};
		...
		case FunctionType::Kind::ArrayPush:
		{
			...
			m_context << u128(1) << Instruction::ADD;
		...
		case FunctionType::Kind::ObjectCreation:
		{
			...
			arguments[0]->accept(*this);
			utils().convertType(*arguments[0]->annotation().type, IntegerType(128));
			...
			m_context << Instruction::MUL << u128(16) << Instruction::ADD;
			...
			{
				m_context << Instruction::DUP2 << u128(16) << Instruction::ADD;
...
bool ExpressionCompiler::visit(MemberAccess const& _memberAccess)
{
	...
	case Type::Category::Magic:
		...
		if (member == "coinbase")
			...
			m_context << u128(0) << Instruction::CALLDATASIZE;
		else if (member == "sig")
			m_context << u128(0) << Instruction::CALLDATALOAD
				<< (u128(0xffffffff) << (128 - 32)) << Instruction::AND;
		else
	...
	case Type::Category::Struct:
	{
		...
		{
			pair<u128, unsigned> const& offsets = type.storageOffsetsOfMember(member);
			m_context << offsets.first << Instruction::ADD << u128(offsets.second);
...
bool ExpressionCompiler::visit(IndexAccess const& _indexAccess)
{
	...
	if (baseType.category() == Type::Category::Mapping)
	{
		...
		{
			m_context << u128(0); // memory position
		...
		m_context << Instruction::KECCAK256 << Instruction::SWAP1 << Instruction::POP;
		m_context << u128(0);
	...
	else if (baseType.category() == Type::Category::Array)
	{
		...
		_indexAccess.indexExpression()->accept(*this);
		utils().convertType(*_indexAccess.indexExpression()->annotation().type, IntegerType(128), true);
	...
	else if (baseType.category() == Type::Category::FixedBytes)
	{
		...
		m_context << u128(fixedBytesType.numBytes());
		m_context << Instruction::DUP2 << Instruction::LT << Instruction::ISZERO;
		// out-of-bounds access throws exception
		m_context.appendConditionalInvalid();

		if (fixedBytesType.numBytes() > 16) {
			// if index >= 16
			m_context << Instruction::DUP1 << u128(16) << Instruction::GT;
			eth::AssemblyItem tag = m_context.appendConditionalJump();
			m_context << u128(16) << Instruction::SWAP1 << Instruction::SUB;
			m_context << Instruction::SWAP1 << Instruction::SWAP2 << Instruction::SWAP1;
			// end if

			m_context << tag;
			m_context << Instruction::BYTE;
			utils().leftShiftNumberOnStack(128 - 8);
...
{
	solAssert(_type.sizeOnStack() == 1 || _type.sizeOnStack() == 2, "Comparison of multi-slot types.");
	...
	else
	{
		...
		else
		{
			...
			if (auto type = dynamic_cast<IntegerType const*>(&_type))
				isValidType = type->isAddress();
			if (auto type = dynamic_cast<FixedBytesType const*>(&_type))
				isValidType = type->sizeOnStack() == 2;
			solAssert(isValidType, "Invalid type for comparison");

			switch (_operator)
			{
			case Token::GreaterThanOrEqual:
				m_context << Instruction::DUP1 << Instruction::DUP4 << Instruction::LT
					<< Instruction::DUP2 << Instruction::DUP5 << Instruction::EQ
					<< Instruction::DUP3 << Instruction::DUP6 << Instruction::LT
					<< Instruction::AND << Instruction::OR
					<< Instruction::ISZERO;
				break;
			case Token::LessThanOrEqual:
				m_context << Instruction::DUP1 << Instruction::DUP4 << Instruction::GT
					<< Instruction::DUP2 << Instruction::DUP5 << Instruction::EQ
					<< Instruction::DUP3 << Instruction::DUP6 << Instruction::GT
					<< Instruction::AND << Instruction::OR
					<< Instruction::ISZERO;
				break;
			case Token::GreaterThan:
			...
			m_context << Instruction::POP << Instruction::POP << Instruction::POP << Instruction::POP;
		}
	}
}
...
void ExpressionCompiler::appendOrdinaryBinaryOperatorCode(Token::Value _operator, Type const& _type)
{
	if (Token::isArithmeticOp(_operator))
		...
		appendBitOperatorCode(_operator, _type);
	else
...
void ExpressionCompiler::appendArithmeticOperatorCode(Token::Value _operator, Type const& _type)
{
	...
	solAssert(type.sizeOnStack() == 1, "Arithmetic operator only works for 128-bit integers");
...
void ExpressionCompiler::appendBitOperatorCode(Token::Value _operator, Type const& _type)
{
	unsigned sizeOnStack = _type.sizeOnStack();
	solAssert(sizeOnStack == 1 || sizeOnStack == 2, "Invalid type size");
	...
	if (sizeOnStack == 1) {
		...
		{
		case Token::BitOr:
			m_context << Instruction::SWAP1 << Instruction::SWAP2 << Instruction::OR;
			m_context << Instruction::SWAP2 << Instruction::OR << Instruction::SWAP1;
			break;
		case Token::BitAnd:
			m_context << Instruction::SWAP1 << Instruction::SWAP2 << Instruction::AND;
			m_context << Instruction::SWAP2 << Instruction::AND << Instruction::SWAP1;
			break;
		case Token::BitXor:
			m_context << Instruction::SWAP1 << Instruction::SWAP2 << Instruction::XOR;
			m_context << Instruction::SWAP2 << Instruction::XOR << Instruction::SWAP1;
			break;
		default:
			solAssert(false, "Unknown bit operator.");
		}
...
void ExpressionCompiler::appendShiftOperatorCode(Token::Value _operator, Type const& _valueType, Type const& _shiftAmountType)
{
	...
	if (c_amountSigned)
	{
		m_context << u128(0) << Instruction::DUP3 << Instruction::SLT;
		m_context.appendConditionalInvalid();
	}

	solAssert(_shiftAmountType.sizeOnStack() == 1, "Invalie shift amount type");
	solAssert(_valueType.sizeOnStack() == 1 || _valueType.sizeOnStack() == 2, "Invalie shift value type");

	if (_valueType.sizeOnStack() == 1)
	{
		switch (_operator)
		{
		case Token::SHL:
			m_context << Instruction::SWAP1 << u128(2) << Instruction::EXP << Instruction::MUL;
			break;
		case Token::SAR:
			m_context << Instruction::SWAP1 << u128(2) << Instruction::EXP << Instruction::SWAP1 << (c_valueSigned ? Instruction::SDIV : Instruction::DIV);
			break;
		case Token::SHR:
		default:
	...
	else
	{
		if (_operator == Token::SHL) {
			...
			m_context << u128(128) << Instruction::SWAP1 << Instruction::LT;
			eth::AssemblyItem trueTag = m_context.appendConditionalJump();

			// shift_value >= 128
			m_context << Instruction::POP << u128(0) << Instruction::SWAP2
					<< u128(128) << Instruction::SWAP1 << Instruction::SUB
					<< u128(2) << Instruction::EXP << Instruction::MUL;
			...
			// shift_value < 128
			m_context << Instruction::DUP3 << u128(2) << Instruction::EXP << Instruction::MUL;
			m_context << Instruction::DUP2 << Instruction::DUP4 << u128(128) << Instruction::SUB;
			m_context << u128(2) << Instruction::EXP << Instruction::SWAP1 << Instruction::DIV << Instruction::OR;
			m_context << Instruction::SWAP2 << u128(2) << Instruction::EXP << Instruction::MUL << Instruction::SWAP1;

			m_context << endTag;
		} else if (_operator == Token::SAR) {
			m_context << Instruction::DUP3;
			m_context << u128(128) << Instruction::SWAP1 << Instruction::LT;
			eth::AssemblyItem trueTag = m_context.appendConditionalJump();
			...
			// shift_value >= 128
			m_context << Instruction::SWAP1 << Instruction::POP
					<< Instruction::SWAP1 << u128(128) << Instruction::SWAP1 << Instruction::SUB
					<< u128(2) << Instruction::EXP << Instruction::SWAP1 << Instruction::DIV
					<< u128(0);
			...
			// shift_value < 128
			m_context << Instruction::SWAP1 << Instruction::DUP3 << u128(2) << Instruction::EXP << Instruction::SWAP1 << Instruction::DIV;
			m_context << Instruction::DUP2 << Instruction::DUP4 << u128(128) << Instruction::SUB;
			m_context << u128(2) << Instruction::EXP << Instruction::MUL << Instruction::OR;
			m_context << Instruction::SWAP2 << u128(2) << Instruction::EXP << Instruction::SWAP1 << Instruction::DIV;
...
)
{
	...
	unsigned contractStackPos = m_context.currentToBaseStackOffset(2 + gasValueSize + selfSize + (_functionType.isBareCall() ? 0 : 1));
	...
	if (funKind == FunctionType::Kind::ECRecover)
	{
		...
		utils().fetchFreeMemoryPointer();
		m_context << u128(0) << Instruction::DUP2 << Instruction::MSTORE;
		m_context << u128(32) << Instruction::ADD;
		utils().storeFreeMemoryPointer();
	}
	...
	if (!_functionType.gasSet() && retSize > 0)
	{
		...
		m_context << u128(retSize) << Instruction::ADD << Instruction::MSTORE;
	}
	...
	// contract address takes two stack items
	...
	else
	{
		...
		u128 gasNeededByCaller = eth::GasCosts::callGas + 10;
	...
	unsigned remainsSize =
		3 + // contract address, input_memory_end
	...
	else if (funKind == FunctionType::Kind::ECRecover)
	{
		...
		m_context << Instruction::SUB;
		// load right-half
		m_context << Instruction::DUP1 << u128(16) << Instruction::ADD << Instruction::MLOAD;
		// load left-half
