...
/// Interprets @a _u as a two's complement signed number and returns the resulting s128.
inline s128 u2s128(u128 _u)
{
	static const bigint c_end = bigint(1) << 128;
	if (boost::multiprecision::bit_test(_u, 127))
		return s128(-(c_end - _u));
	else
		return s128(_u);
}
...
/// @returns the two's complement signed representation of the signed number _u.
inline u128 s2u128(s128 _u)
{
	static const bigint c_end = bigint(1) << 128;
    if (_u >= 0)
		return u128(_u);
    else
		return u128(c_end + _u);
}
