...
contract Controller is Owned, Finalizable, ControllerEventDefinitions {
     ...
     *
     */
    function claimByProof(address _claimer, bytes32[] data, bytes32[] proofs, uint128 number)
        onlyToken
...
contract Ledger is Owned, SafeMath, Finalizable {
     ...
     * @notice: not yet used
     */
    mapping(uint128 => bytes32) public proofs;
     ...
     *
     */
    function multiMint(uint nonce, bytes32[] bits) onlyOwner {
    ...
    function setProof(uint128 _key, bytes32 _proof) onlyController {
        proofs[_key] = _proof;
    }

    function setLocked(address _key, uint128 _value) onlyController {
        locked[_key] = _value;
    }
...
contract Token is Finalizable, TokenReceivable, SafeMath, EventDefinitions, Pausable {
     ...
     */
    function controllerBurn(address _from, bytes32 _to, uint128 _value) onlyController {
        Burn(_from, _to, _value);
    }

    function controllerClaim(address _claimer, uint128 _value) onlyController {
     ...
     *
     */
    function claimByProof(bytes32[] data, bytes32[] proofs, uint128 number) notPaused burnEnabled returns (bool success) {
