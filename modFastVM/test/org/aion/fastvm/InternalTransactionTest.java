...
*/
    ...
    public void testLogs() throws InterruptedException {
        ...
        BlockContext context = bc
            .createNewBlockContext(bc.getBestBlock(), List.of(tx1, tx2), false);
        ...
        AionTransaction tx4 = new AionTransaction(
            ...
            addressA,
            new byte[0],
            ByteUtil.merge(ByteUtil.hexStringToBytes("0x2d7df21a"), addressB.toBytes(),
...
contract A {
    ...
    public void testRecursiveCall() throws InterruptedException {
        ...
        TransactionExecutor exec = new TransactionExecutor(tx2, context.block,
            bc.getRepository().startTracking(), LOGGER_VM);
        ...
        assertArrayEquals(summary.getInternalTransactions().get(0).getHash(),
...
contract A {
    ...
    public void testNestedCreate() throws InterruptedException {
        ...
        BlockContext context = bc.createNewBlockContext(bc.getBestBlock(), List.of(tx1), false);
        TransactionExecutor exec = new TransactionExecutor(tx1, context.block,
            bc.getRepository().startTracking(), LOGGER_VM);
        exec.setExecutorProvider(new TestVMProvider());
        AionTxExecSummary summary = exec.execute();
