...
bool LowerEVMPass::runOnBasicBlock(llvm::BasicBlock& _bb)
{
	...
	auto i256Ty = llvm::IntegerType::get(_bb.getContext(), 256);
	for (auto it = _bb.begin(); it != _bb.end(); ++it)
	{
		...
		if (inst.getType() == Type::Word)
		{
			...
			case llvm::Instruction::UDiv:
				func = Arith128::getUDiv128Func(*module);
				break;

			case llvm::Instruction::URem:
				func = Arith128::getURem128Func(*module);
				break;

			case llvm::Instruction::SDiv:
				func = Arith128::getSDiv128Func(*module);
				break;

			case llvm::Instruction::SRem:
				func = Arith128::getSRem128Func(*module);
				break;
			}
		}
		else if (inst.getType() == i256Ty)
		{
			switch (inst.getOpcode())
			{
			case llvm::Instruction::URem:
				func = Arith128::getURem256Func(*module);
				break;
			}
		}
